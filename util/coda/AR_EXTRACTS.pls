...
-- TODO: Factor duplicate subqueries for consistency and maintenance
  PROCEDURE CUSTOMER_MASTER_FULL (CUSTOMER_MASTER_CUR OUT TYPES.CURSORTYPE) AS
  BEGIN
  ...
  PROCEDURE OPEN_ITEMS_FULL(OPEN_ITEM_CUR OUT TYPES.CURSORTYPE) AS
  BEGIN
  ...
  PROCEDURE OPEN_ITEMS_DELTA(P_START_DATE DATE, P_DAYS INT, OPEN_ITEM_CUR OUT TYPES.CURSORTYPE) AS
  ...
  PROCEDURE ACCT_BALANCES_NET(P_PERIOD INT, P_YEAR INT, OPEN_ITEM_CUR OUT TYPES.CURSORTYPE) AS
  BEGIN
    OPEN OPEN_ITEM_CUR FOR
      SELECT
        P_YEAR AS BAL_YR,
...
--      AND CURFLAG = 138
  ...
  PROCEDURE SETTLED_ITEMS_DAYS(P_START_DATE DATE, P_DAYS INT, SETTLED_ITEM_CUR OUT TYPES.CURSORTYPE) AS
   V_NUM_DAYS INT := 1; -- Minimum of one day
  BEGIN
  -- Set-up the date interval
  ...
  PROCEDURE SETTLED_ITEMS_RANGE(P_START_DATE DATE, P_END_DATE DATE, SETTLED_ITEM_CUR OUT TYPES.CURSORTYPE) AS
  BEGIN
    OPEN SETTLED_ITEM_CUR FOR
      SELECT 
        PL.CMPCODE,
  ...
  PROCEDURE AR_ITEMS_HISTORY(P_START_DATE DATE, P_DAYS INT, AR_ITEM_CUR OUT TYPES.CURSORTYPE) AS
    V_NUM_DAYS INT := 1; -- Minimum of one day
  BEGIN
  -- Set-up the date interval
    IF P_DAYS < 0 THEN -- A negative value indicates that all records modified since the referenced date should be fetched
