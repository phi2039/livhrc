  ...
  BEGIN
    ...
    -- Single-client, high volume documents (invoices)
      SELECT 
          ...
          (CASE WHEN ORG.EQUIV IS NULL THEN NULL ELSE (CASE WHEN HEAD.CURDOC=CMP.HOMECUR THEN CMP.DUALCUR ELSE CMP.HOMECUR END) END) AS ALTCURR,
          -- Original Amounts
          GET_ORG_MASK(HEAD.DOCCODE) * ORG.VALUEDOC AS ORGVAL_DOC,
          GET_ORG_MASK(HEAD.DOCCODE) * ORG.VALUEHOME AS ORGVAL_HOME,
          (CASE WHEN ORG.EQUIV IS NULL THEN NULL ELSE GET_ORG_MASK(HEAD.DOCCODE) * (CASE WHEN HEAD.CURDOC=CMP.HOMECUR THEN ORG.VALUEDUAL ELSE ORG.VALUEHOME END) END) AS ORGVAL_ALT,
          ...
          (CASE WHEN ORG.EQUIV IS NULL THEN NULL ELSE (CASE WHEN HEAD.CURDOC = CMP.HOMECUR THEN OPN.TOTALDUAL ELSE OPN.TOTALHOME END) END) AS OPENVAL_ALT, -- Alternate currency amount is only valid for clients with equivalent status
          ...
          (CASE WHEN ORG.EQUIV IS NULL THEN NULL ELSE (CASE WHEN HEAD.CURDOC = CMP.HOMECUR THEN PAY.TOTALDUAL ELSE PAY.TOTALHOME END) END) AS PAYVAL_ALT, -- Alternate currency amount is only valid for clients with equivalent status
