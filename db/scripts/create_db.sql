...
CREATE INDEX MASTER_DATA.IDX_AR_ITEM_CUST
   ON MASTER_DATA.AR_ITEM_MASTER (CUSTID ASC);
...
CREATE INDEX MASTER_DATA.IDX_AR_SETTLEMENT 
   ON MASTER_DATA.AR_SETTLEMENT_MASTER(CMPCODE, CUSTID, DOCTYPE, DOCNUM, MATCHREF);
...
CREATE CACHED TABLE STAGING.CUST_EXT_LOAD
(
   CUSTID      VARCHAR(32)            NOT NULL,
   NAME        VARCHAR(1024),
   ADD1        VARCHAR(1024),
...
ALTER TABLE STAGING.CUST_EXT_LOAD
   ADD PRIMARY KEY (CUSTID);

CREATE CACHED TABLE STAGING.CUST_OVER_LOAD
(
   CUSTID      VARCHAR(32)            NOT NULL,
   NAME        VARCHAR(1024),
   ADD1        VARCHAR(1024),
...
ALTER TABLE STAGING.CUST_OVER_LOAD
   ADD PRIMARY KEY (CUSTID);
